...
use crate::{
    constants::MAP_SIZE, input::player_action::PlayerAction, plugins::player::Player, Map,
};

use super::{move_ability::MoveAbility, path::Path};
...
pub fn move_player(
    ...
    mut query: Query<(&mut TilePos, &mut MoveAbility), With<Player>>,
    ...
    for (mut tile_pos, mut last_moved_time) in query.iter_mut() {
        ...
        let passed_threshold = current_time - last_moved_time.last_moved > MOVE_COOLDOWN;
        ...
        if walkable_query.get(next).is_ok() && passed_threshold {
            ...
            last_moved_time.last_moved = current_time;
